 ...
 *
 * Reads Qwen Code session history (read-only) to extract user messages
...
async function cmdScanSessions(overridePath, options, raw) {
  ...
  if (!sessionsDir) {
    ...
    error(`No Qwen Code sessions found at ${searchedPath}.${overridePath ? '' : ' Is Qwen Code installed?'}`);
  }
