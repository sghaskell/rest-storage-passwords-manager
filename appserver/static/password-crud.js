...
require([
    ...
    var passwordSearch = new SearchManager(
      {
        ...
        sample_ratio: 1,
        search:
          "| rest /servicesNS/-/-/storage/passwords splunk_server=local \
                | search title=" +
          row.realm +
          ":" +
          row.username +
  ...
  function refreshWindow() {
    ...
    // Small delay to allow modal to close before refreshing
    setTimeout(function () {
      populateTable();
    }, 300);
  }
  ...
  function clearOnClickAndRegister(el, callback, callbackArgs = null) {
    ...
    if (
      _.isObject($._data($(el).get(0), "events")) &&
      _.has($._data($(el).get(0), "events"), "click")
    ) {
      $(el).off("click");
  ...
  function getIdSelections() {
    return $.map(
      $("#rest-password-table").bootstrapTable("getSelections"),
      function (row) {
        return row;
      },
    );
  ...
  function populateTable() {
    ...
    // Always dispose existing search manager for fresh data
    var existingSearch = mvc.Components.get("search1");
    if (existingSearch) {
    ...
    var search1 = new SearchManager(
      {
        ...
        sample_ratio: 1,
        search:
          '| rest /servicesNS/-/-/configs/conf-passwords splunk_server=local \
    ...
    // Clear existing content before starting the search
    $(passwordTableDiv).empty();
    $(contextMenuDiv).empty();

    // Flag to track if data has been rendered
    ...
    search1.on("search:done", function (properties) {
      // Only render empty table if no data was received
      if (!dataRendered && properties.content.resultCount == 0) {
        $(passwordTableDiv).empty();
        var noData = null;
  ...
  function createTable(tableDiv, contextMenuDiv, data) {
    ...
    $("#rest-password-table").bootstrapTable({
      ...
      onExpandRow: function (index, row, $detail) {
        ...
        $detail
          .html("<table></table>")
          .find("table")
          .append('<tr><td><div id="' + row.username + '"></div></td></tr>');
    ...
    $("#rest-password-table").on(
      "check.bs.table uncheck.bs.table " +
        "check-all.bs.table uncheck-all.bs.table",
      function () {
        $("#remove").prop(
          "disabled",
          !$("#rest-password-table").bootstrapTable("getSelections").length,
        );
      },
    );
  ...
  function deleteMultiCredential(rows) {
    ...
    var deleteCred = function (row) {
      ...
      var deleteUrl =
        ...
        ":";
      var message = [];
      var payload = { body: undefined, responseCode: undefined };
      ...
      $.ajax({
        ...
        error: function (xhr, textStatus, error) {
          payload.body =
            '<div class="alert alert-error"><i class="icon-alert"></i>Failed to delete user <b> ' +
            row.username +
            "</b> - " +
            xhr.responseText +
            "</div>";
        ...
        $.ajax({
          ...
          success: function (data, textStatus, xhr) {
            payload.body =
              '<div><i class="icon-check-circle"></i> Successfully deleted credential - <b>' +
              row.realm +
              ":" +
              row.username +
              "</b></div>";
    ...
    // Render delete confirmation modal and register delete callback action
    var deleteUser = renderModal(
      "user-delete-confirm",
      "Confirm Delete Action",
      '<div class="alert alert-error"><i class="icon-alert"></i>You\'re about to remove the users <b>' +
        users.join(", ") +
        "</b> - Press ok to continue</div>",
      "Ok",
      removeUsers,
      [rows],
    );
  ...
  function splunkJSInput(config) {
    ...
    this.renderComponent = function () {
      ...
      // Get search manager
      var splunkJsComponentSearch = mvc.Components.get(
        this.config.id + "-search",
      );
      ...
      if ($(el).length) {
        ...
        if (this.config.type == "dropdown") {
          this.config.instance = new DropdownView({
            id: this.config.id,
            managerid: _.isUndefined(this.config.searchString)
              ? null
              : this.config.id + "-search",
            choices: choices,
            labelField: "label",
  ...
  function renderCreateUserForm(cUsername = false, cRealm = false) {
    var createUser = function createUser() {
      ...
      _.each(arguments[2], function (component, i) {
        ...
        aclData[aclKey] = _.isArray(component.getVals())
          ? component.getVals().join()
          : component.getVals();
        formVals[aclKey] = _.isArray(component.config.default)
          ? component.config.default.join()
      ...
      var createData = { name: username, password: password, realm: realm };
      ...
      if (password != confirmPassword) {
        ...
        $.ajax({
          ...
          success: function (data, textStatus, xhr) {
            message.push(
              '<div><i class="icon-check-circle"></i> Successfully created user <b>' +
                realm +
                ":" +
                username +
                "</b></div>",
          ...
          error: function (xhr, textStatus, error) {
            message.push(
              '<div class="alert alert-error"><i class="icon-alert"></i>Failed to create user <b>' +
                username +
                ":" +
                realm +
                "</b> - " +
          ...
          .then(function () {
            ...
            if (aclData.sharing == "user") {
              ...
              return $.ajax({
                ...
                error: function (xhr, textStatus, error) {
                  message.push(
                    '<div class="alert alert-error"><i class="icon-alert"></i>Failed to apply ACL - ' +
                      xhr.responseText +
                      "</div>",
                  );
                },
          ...
          .then(function () {
            return $.ajax({
              ...
              success: function (data, textStatus, xhr) {
                message.push(
                  '<div><i class="icon-check-circle"></i> Successfully applied ACL\'s</div>',
                );
              },
              error: function (xhr, textStatus, error) {
                message.push(
    ...
    var inputs = [
      new splunkJSInput({
        id: "app-scope-dropdown",
        searchString:
          "| rest /servicesNS/-/-/apps/local splunk_server=local | search disabled=0 | rename title as value | table label, value",
      ...
      new splunkJSInput({
        id: "read-user-multi",
        searchString:
          "| rest /servicesNS/-/-/authorization/roles splunk_server=local | eval label=title | rename title as value | fields label, value | append [| rest /servicesNS/-/-/authentication/users splunk_server=local | eval label=title | rename title as value | fields label, value] | dedup label",
        el: "read-user-multi",
        type: "multi-dropdown",
        default: "*",
        choices: [{ label: "*", value: "*" }],
      ...
      new splunkJSInput({
        id: "owner-dropdown",
        searchString:
          "| rest /servicesNS/-/-/authentication/users splunk_server=local | eval label=title | rename title as value | table label, value",
        el: "owner-dropdown",
        type: "dropdown",
        aclKey: "owner",
    ...
    clearOnClickAndRegister("#create-submit", createUser, [
      cUsername,
      cRealm,
      inputs,
    ]);
  ...
  function renderUpdateUserInTable(row) {
    var updateUser = function updateUser() {
      ...
      // Add realm to formVals for reference in REST url's
      ...
      if (password != confirmPassword) {
        ...
        $.ajax({
          ...
          error: function (xhr, textStatus, error) {
            message.push(
              '<div class="alert alert-error"><i class="icon-alert"></i>Failed to update password for user <b>' +
                username +
                "</b> - " +
                xhr.responseText +
                "</div>",
          ...
          .then(function () {
            ...
            if (password) {
              ...
              return $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                  message.push(
                    '<div><i class="icon-check-circle"></i> Successfully updated password for credential - <b>' +
                      formVals.realm +
                      ":" +
                      username +
                      "</b></div>",
          ...
          .then(function () {
            ...
            if (formVals.app != aclApp) {
              ...
              return $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                  message.push(
                    '<div><i class="icon-check-circle"></i> Successfully moved credential from <b>' +
                      formVals.app +
                      "</b> to <b>" +
                      aclApp +
                      "</b></div>",
                ...
                error: function (xhr, textStatus, error) {
                  message.push(
                    '<div class="alert alert-error"><i class="icon-alert"></i>Failed to move credential from <b>' +
                      formVals.app +
                      "</b> to <b>" +
                      aclApp +
                      "</b> - " +
        ...
        $.ajax({
          ...
          error: function (xhr, textStatus, error) {
            return renderModal(
              ...
              '<div class="alert alert-error"><i class="icon-alert"></i>Failed to view password - ' +
                xhr.responseText +
                "</div>",
              "Close",
            );
